...
private:
	...
	std::shared_ptr<MyTreeNode<T>> central;
	MyTreeNode<T> *parent;//обычный указатель для того, чтобы 2 shared_ptr'a не ссылались друга на друга
	T key;

public:
	MyTreeNode(const T, MyTreeNode<T>* = nullptr, const std::shared_ptr<MyTreeNode<T>>& = nullptr, const std::shared_ptr<MyTreeNode<T>>& = nullptr, const std::shared_ptr<MyTreeNode<T>>& = nullptr);
	~MyTreeNode();
	T& GetKey();
	...
	MyTreeNode<T>* GetCentralNode()const noexcept;
};
...
class MyTree
{
	constexpr enum nodepos { left, right, nil, central };
private:
	...
	size_t RecursiveGetHeight(const MyTreeNode<T>* = nullptr, const bool &ignorecentral = false)const noexcept;
...
public:
	...
	void RemoveNode(MyTreeNode<T>*) noexcept;
	...
	MyTreeNode<T>* GetCentralNode(const MyTreeNode<T>*)const noexcept;

	bool IsEmpty()const noexcept;
	size_t GetNodesCount()const noexcept;
	size_t GetHeight(const bool &ignorecentral = false)const noexcept;
